...
function Dashboard(props) {
        ...
        return (
            <div key={e.name}>
                ...
                {e.events.map((event) => 
                    ...
                    <button 
                        onClick={() => deleteDay(e.trip_id)}>Delete Day</button>
            </div> )})

    const emptyDays = dayLinks.map( (e, index) => <Link to={`/Trip/${e.trip_id}`} key={index}><h6>Day {index+1}</h6></Link>)
    ...
    <div className="dashboardmain">
        ...
        <div className="dashboarddays">
            ...
            <h6>or</h6>
            <button 
                onClick={() => newlist()}>Add Day to Trip
            </button>
        </div>
