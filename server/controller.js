...
module.exports = {
  ...
  retrieveAttractions: (req, res) => {
    ...
    db.get_attractions().then((attractions) =>
  ...
  retrieveAttraction: (req, res) => {
    ...
    db.get_attraction(id).then((attraction) => {
      let total = 0;
      let combined = {
        name: attraction[0].name,
        location: attraction[0].location,
        description: attraction[0].description,
        photo: attraction[0].photo,
        comments: [],
      };
  ...
  editRating: async (req, res) => {
    ...
    if (existingRating[0]) {
      db.edit_survey([user_id, attraction_id, rating, comments]).then(() =>
        res.sendStatus(200)
      );
    } else {
      db.new_survey([attraction_id, user_id, rating, comments]).then(() =>
        res.sendStatus(200)
      );
    }
  },
  ...
  addEvent: (req, res) => {
    ...
    db.add_event([trip_id, attraction_id, user_id]).then(res.sendStatus(200));
  },
  ...
  deleteFromTrip: (req, res) => {
    ...
    db.remove_attraction(id).then(() => res.sendStatus(200));
  },
  ...
  deleteDay: (req, res) => {
    ...
    db.delete_day(id).then(() => res.sendStatus(200));
  },
  ...
  getUserRatings: (req, res) => {
    ...
    db.get_user_ratings(id).then((ratings) => {
      ratings.sort((a, b) =>
        a.rating.length > b.rating.length ? -1 : a.rating < b.rating ? 1 : -1
      );
  ...
  tripDate: async (req, res) => {
    ...
    db.edit_trip_date([id, date]).then((user) => {
      req.session.user = {
        id: user[0].user_id,
        username: user[0].username,
        email: user[0].email,
